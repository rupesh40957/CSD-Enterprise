...
import { Plus, Edit2, Trash2, Save, X, Award, CheckCircle2, AlertCircle, RefreshCw } from "lucide-react";
...
export default function CertificationsManager() {
  ...
  const fetchCertifications = async () => {
    ...
    try {
      ...
      setFeedback({ type: "error", message: "Failed to load certifications." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setFeedback({ type: "success", message: `Certification ${isNew ? "added" : "updated"} successfully!` });
        setEditing(null);
        fetchCertifications();
      } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage ISO, PESO, MSME, and statutory safety approvals displayed in trust badges.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {certifications.map((cert) => (
          <div
            ...
            className="p-5 rounded-2xl bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-800 shadow-sm flex items-start justify-between gap-4"
          >
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-xl bg-cyan-500/15 text-cyan-500 flex items-center justify-center shrink-0 mt-0.5">
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-400 mt-2 font-mono">
                  Order #{cert.sortOrder} • {cert.isActive ? "Active" : "Disabled"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                type="button"
                onClick={() => handleDelete(cert._id ? String(cert._id) : undefined)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-cyan-500/30 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <ImageUploader
                label="Certificate / Compliance Badge Image"
                value={editing.logo || ""}
                onChange={(url) => setEditing({ ...editing, logo: url })}
                helperText="Upload official badge or emblem (e.g., ISO 9001, PESO, MSME, BIS)"
                previewHeight="h-24"
              />
